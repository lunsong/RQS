...
void print(double r_ratio,
	   ...
	   double Omega, double Omega_K, double J, double Mass_p
	   )
{
  ...
  printf(
	 "%4.10f \t%4.10f \t%4.10f \t%4.10f \t%4.10f \t%5.10f \t%5.10f \t%5.10f \t%5.10f \t%5.10f\n",
	 r_ratio,
	 e_center,
	 Mass/MSUN,
	 Mass_0/MSUN,
         ...
         char **argv)                 /* Command line arguments */
{
 ...
 float ans, fh,fl,fm,fnew,sroot,xh,xl,xm,xnew,xacc;
  ...
  printf("ratio\te_15\tM\tM_0\tr_star\tspin\tOmega_K\tI\tJ/M^2\tM_p\n");
  if(strcmp(eos_type,"tab")==0)
    printf("\tg/cm^3\tsun\tsun\tkm\ts-1\ts-1\tg cm^2\tsum\n");
  printf("\n");
  ...
  mass_radius( s_gp, mu, log_e_tab, log_p_tab, 
	       ...
	       &Mass, &Mass_0, &J, &R_e, v_plus, v_minus, &Omega_K, &Mass_p);


  /* PRINT OUT INFORMATION ABOUT THE STELLAR MODEL */

  if(strcmp(eos_type,"tab")==0)
    print(r_ratio,e_center, Mass, Mass_0, R_e, Omega, Omega_K, J, Mass_p);
  else 

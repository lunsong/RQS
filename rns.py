...
from scipy.optimize import fsolve, broyden1, basinhopping

from units import msol, cm, g, s
from quark_eos import load_eos, load_quark_eos, c, Mb

class RNS:
    def __init__(self, eos, MDIV=101, SDIV=201, compilation=False):
        SMAX = .9999

        if compilation:
            cmd = f"gcc -fPIC --shared -DMDIV={MDIV} -DSDIV={SDIV} "\
                  f"equil_util.c spin.c nrutil.c -lm -o spin.so"
            if Popen(cmd.split()).wait() != 0:
                raise Exception("compilation failed")

        rns = cdll.LoadLibrary("./spin.so")
        self.rns = rns
    ...
    def spin(self,r_ratio,ec=None,cf=1,acc=1e-5,max_n=100,print_dif=1):
        if not .5<r_ratio<1:
            return
        self.ec = ec
        ...
        self.rns.spin(self.s_gp, self.mu, self.lg_e, self.lg_p, self.lg_h,
                ...
                cf, max_n, print_dif, r_ratio, self.r_e, self.Omega)
    ...
    def find(self, T=None, J=None, M0=None, solve=fsolve):
        if M0==None:
            M0 = self.M0.value
        if T!=None:
            err = lambda: (self.T - T) / (msol*c**2)
        elif J!=None:
            err = lambda: (self.J.value - J) * g*cm**2/s
        else:
            raise ValueError("T and J are both None")

        def obj(x):
            e,r = x
            self.spin(r,e,print_dif=0)
            res = (self.M0.value-M0)/msol, err()
            print(x,res)
            return res

        solve(obj, (self.ec, self.r_ratio))

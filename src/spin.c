...
double n0_at_e(double ee, 
               ...
               int    *n_nearest_pt)
{
 return pow(10.0,interp(log_e_tab,log_n0_tab,n_tab,log10(ee), n_nearest_pt,1));
}
...
double h_at_p(double pp, 
              ...
              int    *n_nearest_pt)
{
 return pow(10.0,interp(log_p_tab,log_h_tab,n_tab,log10(pp), n_nearest_pt,1));
}
...
double p_at_e(double ee, 
              ...
              int    *n_nearest_pt)
{
 return pow(10.0,interp(log_e_tab,log_p_tab,n_tab,log10(ee), n_nearest_pt,1));
} 
...
double p_at_h(double hh, 
              ...
              int    *n_nearest_pt)
{
 return pow(10.0,interp(log_h_tab,log_p_tab,n_tab,log10(hh), n_nearest_pt,1));
}
              ...
              double Gamma_P)
{
 if(strcmp(eos_type,"tab")==0)
   return pow(10.0,interp(log_p_tab,log_e_tab,n_tab,log10(pp), n_nearest_pt,1));
 else
...
double   sum_rho=0.0,         /* intermediate sum in eqn for rho */
  ...
  double old_dif;
  //int cf_step;
  while( (dif> accuracy || n_of_it<2) && n_of_it < max_it) { 
      //printf("it:%d\n", n_of_it);

      //if (dif>old_dif && cf_step>30 && cf>0.001){
	//  cf *= .5;
	//  cf_step
      if(print_dif)
         fprintf(stderr, "diff = %4.3e\n",dif);
      ...
      */        
      ...
      for(s=1;s<=SDIV;s++) {
         for(m=1;m<=MDIV;m++) {
            rho[(s-1)*MDIV+m-1] /= SQ(r_e);
            gama[(s-1)*MDIV+m-1] /= SQ(r_e); 
            alpha[(s-1)*MDIV+m-1] /= SQ(r_e);
            omega[(s-1)*MDIV+m-1] *= r_e;
         }
         rho_mu_0[s]=rho[(s-1)*MDIV+1-1];     
         gama_mu_0[s]=gama[(s-1)*MDIV+1-1];   
         omega_mu_0[s]=omega[(s-1)*MDIV+1-1]; 
         rho_mu_1[s]=rho[(s-1)*MDIV+MDIV-1];  
         gama_mu_1[s]=gama[(s-1)*MDIV+MDIV-1];
      }
      ...
      n_nearest= SDIV/2;
      gama_pole_h=interp(s_gp,gama_mu_1,SDIV,s_p,&n_nearest,0); 
      gama_equator_h=interp(s_gp,gama_mu_0,SDIV,s_e,&n_nearest,0);
      gama_center_h=gama[(1-1)*MDIV+1-1];                    
  
      rho_pole_h=interp(s_gp,rho_mu_1,SDIV,s_p,&n_nearest,0);   
      rho_equator_h=interp(s_gp,rho_mu_0,SDIV,s_e,&n_nearest,0);
      rho_center_h=rho[(1-1)*MDIV+1-1];                      
      ...
      if(r_ratio==1.0) {
        ...
        omega_equator_h=0.0;
            ...
            omega_equator_h=interp(s_gp,omega_mu_0,SDIV,s_e, &n_nearest,0);
      ...
      for(s=1;s<=SDIV;s++) {
         ...
         for(m=1;m<=MDIV;m++) {
            rsm=rho[(s-1)*MDIV+m-1];
            
            if(r_ratio==1.0) 
                velocity_sq[(s-1)*MDIV+m-1]=0.0;
            else 
                velocity_sq[(s-1)*MDIV+m-1]=SQ((Omega_h-omega[(s-1)*MDIV+m-1])*(sgp/(1.0-sgp))
                                  *sin_theta[m]*exp(-rsm*SQ(r_e)));

            if(velocity_sq[(s-1)*MDIV+m-1]>=1.0) 
              velocity_sq[(s-1)*MDIV+m-1]=0.0;

            enthalpy[(s-1)*MDIV+m-1]=enthalpy_min + 0.5*(SQ(r_e)*(gama_pole_h+rho_pole_h
                           -gama[(s-1)*MDIV+m-1]-rsm)-log(1.0-velocity_sq[(s-1)*MDIV+m-1]));
  
            if((enthalpy[(s-1)*MDIV+m-1]<=enthalpy_min) || (sgp>s_e)) {
                  pressure[(s-1)*MDIV+m-1]=0.0;
                  energy[(s-1)*MDIV+m-1]=0.0; 
	    }
            else { if(strcmp(eos_type,"tab")==0) {
                     pressure[(s-1)*MDIV+m-1]=p_at_h(enthalpy[(s-1)*MDIV+m-1], log_p_tab, 
                                           log_h_tab, n_tab, &n_nearest);
                     energy[(s-1)*MDIV+m-1]=e_at_p(pressure[(s-1)*MDIV+m-1], log_e_tab, 
                                      log_p_tab, n_tab, &n_nearest, eos_type,
                                       Gamma_P);
	           }
                   else {
                         rho0sm=pow(((Gamma_P-1.0)/Gamma_P)
                                *(exp(enthalpy[(s-1)*MDIV+m-1])-1.0),1.0/(Gamma_P-1.0));
 
                         pressure[(s-1)*MDIV+m-1]=pow(rho0sm,Gamma_P);

                         energy[(s-1)*MDIV+m-1]=pressure[(s-1)*MDIV+m-1]/(Gamma_P-1.0)+rho0sm;
                   }
            }  
      ...
      for(s=1;s<=SDIV;s++)
         for(m=1;m<=MDIV;m++) {
            rsm=rho[(s-1)*MDIV+m-1];
            gsm=gama[(s-1)*MDIV+m-1];
            omsm=omega[(s-1)*MDIV+m-1];
            esm=energy[(s-1)*MDIV+m-1];
            psm=pressure[(s-1)*MDIV+m-1];
            e_gsm=exp(0.5*gsm);
            e_rsm=exp(-rsm);
            v2sm=velocity_sq[(s-1)*MDIV+m-1];
            mum=mu[m];            
            m1=1.0-SQ(mum);
            sgp=s_gp[s];
            s_1=1.0-sgp;
            ...
            ea=16.0*PI*exp(2.0*alpha[(s-1)*MDIV+m-1])*SQ(r_e);
      ...
      for(s=1;s<=SDIV;s++) 
         for(m=1;m<=MDIV;m++) {
            ...
            rho[(s-1)*MDIV+m-1]=rsm + cf*(sum_rho-rsm);
            gama[(s-1)*MDIV+m-1]=gsm + cf*(sum_gama-gsm);
            omega[(s-1)*MDIV+m-1]=omsm + cf*(sum_omega-omsm);
      ...
      if(fabs(omega[(2-1)*MDIV+1-1])>100.0 || fabs(rho[(2-1)*MDIV+1-1])>100.0 
         || fabs(gama[(2-1)*MDIV+1-1])>300.0) {
         a_check=200; 
         break;
      }
      ...
      if(r_ratio==1.0) {
        for(s=1;s<=SDIV;s++)
           for(m=1;m<=MDIV;m++) {
              rho[(s-1)*MDIV+m-1]=rho[(s-1)*MDIV+1-1];
              gama[(s-1)*MDIV+m-1]=gama[(s-1)*MDIV+1-1];
              omega[(s-1)*MDIV+m-1]=0.0;          
	   }
      }
      ...
      if(SMAX==1.0) {
         for(m=1;m<=MDIV;m++) {
            rho[(SDIV-1)*MDIV+m-1]=0.0;
            gama[(SDIV-1)*MDIV+m-1]=0.0;
            omega[(SDIV-1)*MDIV+m-1]=0.0;
	 }
      } 
      ...
      dgds = malloc(sizeof(double)*SDIV*MDIV);
      //dgdm = dmatrix(1,SDIV,1,MDIV); 
      dgdm = malloc(sizeof(double)*SDIV*MDIV);
      ...
      if(r_ratio==1.0) {
        for(s=1;s<=SDIV;s++)
           ...
           temp8=m1*exp(-2*rho[(s-1)*MDIV+m-1]);
      ...
      for(s=1;s<=SDIV;s++) {
         alpha[(s-1)*MDIV+1-1]=0.0;
         for(m=1;m<=MDIV-1;m++) 
            alpha[(s-1)*MDIV+m+1-1]=alpha[(s-1)*MDIV+m-1]+0.5*DM*(da_dm[s][m+1]+
                          da_dm[s][m]);
      } 
   ...
   //free_dmatrix(dgdm,1,SDIV,1,MDIV);
      ...
      for(s=1;s<=SDIV;s++)          
         for(m=1;m<=MDIV;m++) {     
            alpha[(s-1)*MDIV+m-1] += -alpha[(s-1)*MDIV+MDIV-1]+0.5*(gama[(s-1)*MDIV+MDIV-1]-rho[(s-1)*MDIV+MDIV-1]);

            if(alpha[(s-1)*MDIV+m-1]>=300.0) {
              a_check=200; 
              break;
            }
            omega[(s-1)*MDIV+m-1] /= r_e;
...

{
 ...
 for(s=1;s<=SDIV;s++) {
    ...
    if(r_is_s<r_is_final) {
      lambda_s=interp(r_is_gp,lambda_gp,RDIV,r_is_s,&n_nearest,0);
      nu_s=interp(r_is_gp,nu_gp,RDIV,r_is_s,&n_nearest,0);
    }
    else {
    ...
    gama[(s-1)*MDIV+1-1]=nu_s+lambda_s;
    rho[(s-1)*MDIV+1-1]=nu_s-lambda_s;

    for(m=1;m<=MDIV;m++) {
        gama[(s-1)*MDIV+m-1]=gama[(s-1)*MDIV+1-1];        
        rho[(s-1)*MDIV+m-1]=rho[(s-1)*MDIV+1-1];
        alpha[(s-1)*MDIV+m-1]=(gama[(s-1)*MDIV+1-1]-rho[(s-1)*MDIV+1-1])/2.0;
        omega[(s-1)*MDIV+m-1]=0.0; 
    }
 
    gama_mu_0[s]=gama[(s-1)*MDIV+1-1];                   /* gama at \mu=0 */
    rho_mu_0[s]=rho[(s-1)*MDIV+1-1];                     /* rho at \mu=0 */

 }

   n_nearest=SDIV/2;
   gama_eq = interp(s_gp,gama_mu_0,SDIV,s_e,&n_nearest,0); /* gama at equator */
   rho_eq = interp(s_gp,rho_mu_0,SDIV,s_e,&n_nearest,0);   /* rho at equator */
...

{
 ...
 double   
   **rho_0, /*rest mass density*/
   ...
   n_nearest= SDIV/2;
   gama_equator=interp(s_gp,gama_mu_0,SDIV,s_e, &n_nearest,0);  
   rho_equator=interp(s_gp,rho_mu_0,SDIV,s_e, &n_nearest,0);   
...
/* Circumferential radius */
 ...
 if(strcmp(eos_type,"tab")==0) {
   ...
   for(s=1;s<=SDIV;s++)
      for(m=1;m<=MDIV;m++) {
           if(energy[(s-1)*MDIV+m-1]>e_surface)
             rho_0[s][m]=n0_at_e(energy[(s-1)*MDIV+m-1], log_n0_tab, log_e_tab, n_tab,
        ...
        for(s=1;s<=SDIV;s++)
           for(m=1;m<=MDIV;m++)
                rho_0[s][m]=(energy[(s-1)*MDIV+m-1]+pressure[(s-1)*MDIV+m-1])*exp(-enthalpy[(s-1)*MDIV+m-1]);
  }
   ...
   for(s=1;s<=SDIV;s++) {
    ...
    for(m=1;m<=MDIV-2;m+=2) {
     D_m[s] += (1.0/(3.0*(MDIV-1)))*( exp(2.0*alpha[(s-1)*MDIV+m-1]+gama[(s-1)*MDIV+m-1])*
              (((energy[(s-1)*MDIV+m-1]+pressure[(s-1)*MDIV+m-1])/(1.0-velocity_sq[(s-1)*MDIV+m-1]))*
              (1.0+velocity_sq[(s-1)*MDIV+m-1]+(2.0*s_gp[s]*sqrt(velocity_sq[(s-1)*MDIV+m-1])/
              (1.0-s_gp[s]))*sqrt(1.0-mu[m]*mu[m])*r_e*omega[(s-1)*MDIV+m-1]*
              exp(-rho[(s-1)*MDIV+m-1])) + 2.0*pressure[(s-1)*MDIV+m-1])

            + 4.0*exp(2.0*alpha[(s-1)*MDIV+m+1-1]+gama[(s-1)*MDIV+m+1-1])*
              (((energy[(s-1)*MDIV+m+1-1]+pressure[(s-1)*MDIV+m+1-1])/(1.0-velocity_sq[(s-1)*MDIV+m+1-1]))*
              (1.0+velocity_sq[(s-1)*MDIV+m+1-1]+(2.0*s_gp[s]*sqrt(velocity_sq[(s-1)*MDIV+m+1-1])/
              (1.0-s_gp[s]))*sqrt(1.0-mu[m+1]*mu[m+1])*r_e*omega[(s-1)*MDIV+m+1-1]*
              exp(-rho[(s-1)*MDIV+m+1-1])) + 2.0*pressure[(s-1)*MDIV+m+1-1]) 

            + exp(2.0*alpha[(s-1)*MDIV+m+2-1]+gama[(s-1)*MDIV+m+2-1])*
              (((energy[(s-1)*MDIV+m+2-1]+pressure[(s-1)*MDIV+m+2-1])/(1.0-velocity_sq[(s-1)*MDIV+m+2-1]))*
              (1.0+velocity_sq[(s-1)*MDIV+m+2-1]+(2.0*s_gp[s]*sqrt(velocity_sq[(s-1)*MDIV+m+2-1])/
              (1.0-s_gp[s]))*sqrt(1.0-mu[m+2]*mu[m+2])*r_e*omega[(s-1)*MDIV+m+2-1]*
              exp(-rho[(s-1)*MDIV+m+2-1])) + 2.0*pressure[(s-1)*MDIV+m+2-1]));    

     D_m_0[s] += (1.0/(3.0*(MDIV-1)))*( exp(2.0*alpha[(s-1)*MDIV+m-1]+(gama[(s-1)*MDIV+m-1]
              -rho[(s-1)*MDIV+m-1])/2.0)*rho_0[s][m]/sqrt(1.0-velocity_sq[(s-1)*MDIV+m-1])

             + 4.0* exp(2.0*alpha[(s-1)*MDIV+m+1-1]+(gama[(s-1)*MDIV+m+1-1]
             -rho[(s-1)*MDIV+m+1-1])/2.0)*rho_0[s][m+1]/sqrt(1.0-velocity_sq[(s-1)*MDIV+m+1-1])
         
             + exp(2.0*alpha[(s-1)*MDIV+m+2-1]+(gama[(s-1)*MDIV+m+2-1]
             -rho[(s-1)*MDIV+m+2-1])/2.0)*rho_0[s][m+2]/sqrt(1.0-velocity_sq[(s-1)*MDIV+m+2-1]));

     D_m_p[s] += (1.0/(3.0*(MDIV-1)))*( exp(2.0*alpha[(s-1)*MDIV+m-1]+(gama[(s-1)*MDIV+m-1]
              -rho[(s-1)*MDIV+m-1])/2.0)*energy[(s-1)*MDIV+m-1]/sqrt(1.0-velocity_sq[(s-1)*MDIV+m-1])
 
              + 4.0* exp(2.0*alpha[(s-1)*MDIV+m+1-1]+(gama[(s-1)*MDIV+m+1-1]
              -rho[(s-1)*MDIV+m+1-1])/2.0)*energy[(s-1)*MDIV+m+1-1]/sqrt(1.0-velocity_sq[(s-1)*MDIV+m+1-1])
          
             + exp(2.0*alpha[(s-1)*MDIV+m+2-1]+(gama[(s-1)*MDIV+m+2-1]
             -rho[(s-1)*MDIV+m+2-1])/2.0)*energy[(s-1)*MDIV+m+2-1]/sqrt(1.0-velocity_sq[(s-1)*MDIV+m+2-1])); 

     D_J[s] += (1.0/(3.0*(MDIV-1)))*( sqrt(1.0-mu[m]*mu[m])*
              exp(2.0*alpha[(s-1)*MDIV+m-1]+gama[(s-1)*MDIV+m-1]-rho[(s-1)*MDIV+m-1])*(energy[(s-1)*MDIV+m-1]
              +pressure[(s-1)*MDIV+m-1])*sqrt(velocity_sq[(s-1)*MDIV+m-1])/(1.0-velocity_sq[(s-1)*MDIV+m-1])
  
              +4.0*sqrt(1.0-mu[m+1]*mu[m+1])*
              exp(2.0*alpha[(s-1)*MDIV+m+1-1]+gama[(s-1)*MDIV+m+1-1]-rho[(s-1)*MDIV+m+1-1])*(energy[(s-1)*MDIV+m+1-1]
              +pressure[(s-1)*MDIV+m+1-1])*sqrt(velocity_sq[(s-1)*MDIV+m+1-1])/
              (1.0-velocity_sq[(s-1)*MDIV+m+1-1])

              + sqrt(1.0-mu[m+2]*mu[m+2])*
              exp(2.0*alpha[(s-1)*MDIV+m+2-1]+gama[(s-1)*MDIV+m+2-1]-rho[(s-1)*MDIV+m+2-1])*(energy[(s-1)*MDIV+m+2-1]
              +pressure[(s-1)*MDIV+m+2-1])*sqrt(velocity_sq[(s-1)*MDIV+m+2-1])/
              (1.0-velocity_sq[(s-1)*MDIV+m+2-1]));
    }
   }
  ...
  for(s=1+(SDIV-1)/2;s<=SDIV;s++) {
    ...
    v_plus[s]=(exp(-rho[(s-1)*MDIV+1-1])*r_e*s_gp[s]*s_gp[s]*d_omega_s + sqrt_v)/
              (2.0+s1*(d_gama_s-d_rho_s));

    v_minus[s]=(exp(-rho[(s-1)*MDIV+1-1])*r_e*s_gp[s]*s_gp[s]*d_omega_s - sqrt_v)/
               (2.0+s1*(d_gama_s-d_rho_s));
  }
...
/* Kepler angular velocity */
   ...
   n_nearest=SDIV/2; 
   doe=interp(s_gp,d_o_e,SDIV,s_e, &n_nearest,0);
   dge=interp(s_gp,d_g_e,SDIV,s_e, &n_nearest,0);
   dre=interp(s_gp,d_r_e,SDIV,s_e, &n_nearest,0);
   //dve=interp(s_gp,d_v_e,SDIV,s_e, &n_nearest,0);
  ...
  if (r_ratio ==1.0)
    ...
    omega_equator = interp(s_gp,omega_mu_0,SDIV,s_e, &n_nearest,0);
